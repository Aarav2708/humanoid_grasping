...
def visualize_grasps(full_pc, pred_grasps_cam, scores, plot_opencv_cam=False, pc_colors=None, gripper_openings=None, gripper_width=0.08):
    ...
    TOP_K = 10  # Limit to top 10 grasps

    for i, k in enumerate(pred_grasps_cam):
        if np.any(pred_grasps_cam[k]):
            all_grasps = pred_grasps_cam[k]
            all_scores = scores[k]
            all_openings = gripper_openings[k] if gripper_openings else np.ones(len(all_grasps)) * gripper_width

            # Get indices of top K scores
            top_k_indices = np.argsort(all_scores)[::-1][:TOP_K]

            top_grasps = all_grasps[top_k_indices]
            top_scores = all_scores[top_k_indices]
            top_openings = all_openings[top_k_indices]

            # Choose colors
            top_colors = [cm2(0.5 * s)[:3] for s in top_scores]

            # Draw only top-K grasps
            draw_grasps(top_grasps, np.eye(4), gripper_openings=top_openings, colors=top_colors)

            # Optionally, highlight the best grasp
            best_idx = top_k_indices[0]
            draw_grasps([all_grasps[best_idx]], np.eye(4),
                        gripper_openings=[all_openings[best_idx]],
